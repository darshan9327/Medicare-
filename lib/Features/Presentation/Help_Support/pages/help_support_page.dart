...
class HelpSupportScreen extends StatelessWidget {
  final List<Map<String, dynamic>> faqItems = [
    ...
    {'question': 'How to cancel an order?', 'answer': 'You can cancel your order within 10 minutes of placing it from the "My Orders" section.'},
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text('Help & Support'), backgroundColor: Color(0xff478ef8), foregroundColor: Colors.white, elevation: 0),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(child: _buildQuickAction(context, '💬', 'Live Chat', 'Chat with our support team', () => _startLiveChat(context))),
                      SizedBox(width: 15),
                      Expanded(child: _buildQuickAction(context, '📞', 'Call Us', '24/7 helpline available', () => _makePhoneCall(context))),
                    ],
                  ),
                  SizedBox(height: 15),
                  Row(
                    children: [
                      Expanded(child: _buildQuickAction(context, '📧', 'Email Us', 'Send us your query', () => _sendEmail(context))),
                      SizedBox(width: 15),
                      Expanded(child: _buildQuickAction(context, '🎥', 'Video Call', 'Video consultation', () => _startVideoCall(context))),
                    ],
                  ),
                ],
              ),
  ...
  Widget _buildQuickAction(BuildContext context, String emoji, String title, String subtitle, VoidCallback onTap) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Color(0xFFF9FAFB), borderRadius: BorderRadius.circular(12), border: Border.all(color: Color(0xFFE5E7EB))),
        child: Column(
          children: [
  ...
  void _makePhoneCall(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Calling +91 1800-123-4567...')));
  }

  void _sendEmail(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Opening email app...')));
  }

  void _startVideoCall(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Starting video consultation...')));
  }
}
...
class _LiveChatScreenState extends State<LiveChatScreen> {
  ...
  List<Map<String, dynamic>> messages = [
    ...
    {'message': 'Hi, I want to know about the side effects of Paracetamol.', 'isUser': true, 'time': '10:32 AM'},
    {
      'message':
          'Paracetamol is generally safe when taken as directed. Common side effects are rare but may include nausea or skin rash. Please consult your doctor if you experience any unusual symptoms.',
      'isUser': false,
      'time': '10:33 AM',
      'senderName': 'Dr. Pharmacist',
    },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Row(
          children: [
            CircleAvatar(radius: 16, backgroundColor: Color(0xFF10B981), child: Icon(Icons.person, color: Colors.white, size: 18)),
            SizedBox(width: 10),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
        ...
        elevation: 0,
        actions: [IconButton(onPressed: () {}, icon: Icon(Icons.videocam)), IconButton(onPressed: () {}, icon: Icon(Icons.call))],
      ),
      body: Column(
        children: [
          Expanded(
          ...
          Container(
            padding: EdgeInsets.all(20),
            decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(blurRadius: 4, color: Colors.black12, offset: Offset(0, -2))]),
            child: Row(
              children: [
                IconButton(onPressed: () => _showAttachmentOptions(context), icon: Icon(Icons.attach_file, color: Color(0xFF6B7280))),
                Expanded(
                  child: Container(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(hintText: 'Type your message...', border: InputBorder.none),
                ...
                Container(
                  ...
                  child: IconButton(onPressed: _sendMessage, icon: Icon(Icons.send, color: Colors.white)),
                ),
              ],
            ),
          ),
  ...
  Widget _buildMessageBubble(Map<String, dynamic> message) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          if (!isUser) ...[
            CircleAvatar(radius: 16, backgroundColor: Color(0xFF10B981), child: Icon(Icons.support_agent, color: Colors.white, size: 16)),
            SizedBox(width: 10),
          ],
          Flexible(
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  if (!isUser && message.containsKey('senderName'))
                    Text(message['senderName'], style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF2563EB))),
                  if (!isUser && message.containsKey('senderName')) SizedBox(height: 4),
                  Text(message['message'], style: TextStyle(color: isUser ? Colors.white : Color(0xFF111827), height: 1.4)),
                  SizedBox(height: 4),
                  Text(message['time'], style: TextStyle(fontSize: 12, color: isUser ? Colors.white70 : Color(0xFF6B7280))),
                ],
              ),
            ),
          ),
      ...
      child: Column(
        children: [
          Container(
            width: 60,
            height: 60,
            decoration: BoxDecoration(color: Color(0xFFF3F4F6), borderRadius: BorderRadius.circular(15)),
